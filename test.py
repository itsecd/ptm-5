...
def test_square(a, b, c, expected):
    assert square_eq_solver(a, b, c) == expected

@pytest.mark.parametrize("x, y, expr, expected", [('x', 'y', '5 * x**2 - 3 * y', ('10x', -3)), ('x', 'y', '18 + 332',(0, 0)), ('x', 'y', '182 * x', (182, 0))])
def test_derivatives(x, y, expr, expected):
    assert derivatives(x, y, expr) == expected

@pytest.mark.parametrize("x, expr, expected", [('x', 'cos(x)', 'sin(x)'), ('x', 'x - 1', 'x**2 - x'), ('x', 'sqrt(x)', '2*x**(3/2)/3')])
def test_indefinite_integral(x, expr, expected):
...
def test_commission(a, b, c, d, expected):
    assert commission(a, b, c , d) == expected

@pytest.mark.parametrize("a, b, c, expected", [(0, 1, 0, 0.0), (32, 2, 3, 96.0), (18, 18, 18, 324.0)])
def test_trapezoid_area(a, b, c, expected):
